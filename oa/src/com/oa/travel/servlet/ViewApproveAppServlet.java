...
public class ViewApproveAppServlet extends BaseServlet {

	/**
	 * Constructor of the object.
	 */
	public ViewApproveAppServlet() {
		super();
	}
...
TravelAppServiceImpl travelAppServiceImpl = new TravelAppServiceImpl();
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		List<WorkStream> workStreamsList=workStreamServiceImpl.findAppedByBossId(empId, 5, pageNo);
		
		List<TravelApp> travelAppList=travelAppServiceImpl.searchApproveApp(workStreamsList);
	
		int pageCount = workStreamServiceImpl.computePageCount(userInfo.getEmpId(), 5,true,"SQ");
		request.setAttribute("travelAppList", travelAppList);
		request.setAttribute("pageNo", pageNo);
		request.setAttribute("pageCount", pageCount);
		
		request.getRequestDispatcher("/html/worktime/d3.jsp").forward(request, response);
	}
