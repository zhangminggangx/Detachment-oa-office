...
public class ViewUnApproveAppServlet extends BaseServlet {

	/**
	 * Constructor of the object.
	 */
	public ViewUnApproveAppServlet() {
		super();
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		List<WorkStream> workStreamsList=workStreamServiceImpl.findAppByBossId(empId, 100, pageNo);
		
		List<TravelApp> travelAppList=travelAppServiceImpl.searchApproveApp(workStreamsList);
		int total=travelAppList.size();
			int pageCount=workStreamServiceImpl.computePageCount(empId, 100, false,"SQ");
		request.setAttribute("travelAppList", travelAppList);
		request.setAttribute("pageNo", pageNo);
		request.setAttribute("pageCount", pageCount);
		
		request.getRequestDispatcher("/html/worktime/d1.jsp").forward(request, response);
